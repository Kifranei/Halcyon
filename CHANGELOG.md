...
From 1.2.9-beta1 to 1.2.9-beta2.

中文更新日志
- USB DAC「独占模式」改为真正 AAudio Exclusive 开流：校验 SharingMode，拒绝静默 Shared；bit-perfect（不做 SRC）；失败时仍挂在 DAC 上 Shared 回退；设置页显示独占中 / 仅路由 / 失败。
- 底栏默认开启搜索合并，搜索作为第五页；合并开启时自动补齐搜索项，并修复旧的 4 项上限导致塞不进搜索的问题。
- 歌词页默认：非当前行模糊 70%、歌词 32sp、翻译/罗马音 16sp（已改过的本地设置不会被覆盖）。
- 搜索页接入壁纸 / 当前播放流光背景。
- 最近播放（#653）：按日分组（含「今日」）、歌曲行 SongItem、多选添加歌单走专辑式 AddToPlaylistSheet；计数改为「条」。
- 播放页下滑关闭改为跟手 1:1；歌词页进入时隐藏操作栏，底部约 1/3 区域点击显示。
- #657 远程音频缓存 / 批量下载入口完善。多选顶栏的下载按钮不再被双击回顶区域挡住；Navidrome / OpenSubsonic 批量缓存会按当前服务器重新签发播放地址。
- #658 文件夹作专辑名开关修复；动态封面从歌词/播放列表返回时续播。
- 移除预测性返回残留代码。
- 艺术家详情页动态封面（及专辑沉浸头图）对齐 Apple Music 软溶边：DstIn 底部淡出；有动态视频时头图加高到 560dp、溶边更早；尊重动态封面开关。
- 逐字歌词支持「彩虹色歌词」：当前词高亮使用七彩光谱（赤橙黄绿青蓝紫），羽化边缘的色标保持递增；非当前行保持原色。
- 「歌词 HDR 高亮（长音）」默认关闭。开启后，卡拉 OK 高亮与长音辉光按约 1.5× 提亮（SDR 亮度增强）。
- 最近播放按页签记住条数上限和「显示日期」，界面重组后不再回到默认值；保存的键值按每一条切分，避免串到别的页签。

English Changelog
- USB DAC exclusive mode now opens true AAudio Exclusive (reject silent Shared), bit-perfect without SRC, Shared+pinned fallback on failure, and settings status for exclusive / routing-only / failed.
- Bottom dock: search merge on by default with Search as the 5th tab; auto-append Search when merge is on; fix the old 4-item cap that blocked adding Search.
- Lyrics defaults: non-current blur 70%, lyric 32sp, translation/romaji 16sp (existing DataStore values kept).
- Search page participates in wallpaper / now-playing flow backgrounds.
- Recently played (#653): day groups (Today), SongItem rows, multi-select add-to-playlist via album-style sheet; count uses entries.
- Playback dismiss is 1:1 follow-hand; lyrics page hides chrome on enter and reveals on bottom ~⅓ tap.
- #657 remote audio cache / batch download wiring. The multi-select download button is no longer covered by the double-tap-to-top overlay, and Navidrome / OpenSubsonic batch cache reissues the stream URL from the current server.
- #658 folder-as-album-name switch; dynamic cover resumes after leaving lyrics/playlist.
- Strip predictive-back leftovers.
- Artist detail (and album immersive) dynamic covers use AM soft melt (DstIn), taller 560dp hero when video plays, earlier fade; honor dynamic-cover switch.
- Rainbow lyrics paint the active karaoke word with a seven-color spectrum (red, orange, yellow, green, cyan, blue, violet) and keep feather stops ordered; other lines stay the normal color.
- Lyric HDR highlight for sustained notes is off by default. When on, karaoke fill and sustain glow brighten about 1.5× in SDR.
- Recently played remembers each tab’s entry limit and show-date choice across recomposition, and splits stored key/value records per entry so one tab does not read another tab’s setting.

Version
- Version name: `1.2.9-beta2`
- Version code: `38`
