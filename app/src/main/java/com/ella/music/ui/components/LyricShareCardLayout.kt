...
internal fun calculateLyricShareLayout(
    ...
    val maxLyricsHeight = (
        maxHeight - lyricMargin * 2 - chinHeight
    ...
    val lyricsTop = lyricMargin
    val chinTop = lyricsTop + lyricsHeight + lyricMargin
    val adaptiveCanvasHeight = (chinTop + chinHeight).roundToInt()
        .coerceIn(minHeight.coerceAtLeast(1), maxHeight)

    val coverCenterY = chinTop + chinHeight / 2f
    val coverRect = RectF(
        padding,
    ...
    val stackHeight = titleLayout.height + artistLayout.height + logoGap + logoHeight
    val stackTop = chinTop + (chinHeight - stackHeight) / 2f
