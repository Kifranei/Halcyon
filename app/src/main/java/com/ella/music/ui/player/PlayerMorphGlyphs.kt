...
/**
 * Apple Music 队列/待播清单图标（list.bullet）：三条圆角水平横线与左侧对应圆点。
 */
@Composable
internal fun AppleQueueIcon(
    ...
    Canvas(modifier = modifier) {
        ...
        if (s <= 0f) return@Canvas
        val stroke = s * 0.08f
        val dotRadius = s * 0.045f
        val dotX = s * 0.18f
        val lineStartX = s * 0.34f
        val lineEndX = s * 0.84f
        val yOffsets = listOf(s * 0.28f, s * 0.50f, s * 0.72f)
        for (y in yOffsets) {
            drawCircle(
                color = color,
                radius = dotRadius,
