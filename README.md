...
- 艺术家匹配与多源信息：支持自定义本地艺术家文件夹匹配，自动关联图片与视频背景；支持自定义艺术家分割符号（如 `/`、`;`、`&`、`、` 等），精确拆分与聚合合作曲目；支持从 Wikipedia、Last.fm、网易云等多数据源并发拉取艺术家简介与多语言信息，内置 Cloudflare 验证交互 Sheet 以便安全重试。
- 艺术家静态封面与动态背景按自定义资源、独占/合作专辑艺术家、独占/合作歌曲艺术家的优先级智能匹配；支持多张封面轮播与 MP4/MKV/WebM/MOV 视频背景，同时包含图片与视频时优先播放视频。
- 专辑详情页支持独立的专辑介绍页面和手动编辑，支持一键从网易云拉取专辑介绍；本地专辑优先写入同目录 `album.nfo` 的 `<review>`，目录不可写时自动保存到应用内。
